...
class WarehouseRobotEnv(gym.Env):
    ...
    def __init__(self, grid_rows=4, grid_cols=5, render_mode=None):
        ...
        self.observation_space = spaces.Box(
            low=0,
            high=np.array([self.grid_rows - 1, self.grid_cols - 1,  # Robot position
                        self.grid_rows - 1, self.grid_cols - 1,  # Target position
                        self.grid_rows - 1, self.grid_cols - 1,  # Package position
                        1]),  # Package picked status: 0 or 1
            shape=(7,),  # Updated shape
            dtype=np.int32
        )
    ...
    def step(self, action):
        ...
        if not self.warehouse_robot.package_picked and self.warehouse_robot.robot_pos == self.warehouse_robot.package_pos:
            reward = 0.5
