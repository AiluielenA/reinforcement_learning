...
class WarehouseRobot:
    ...
    def __init__(self, grid_rows=4, grid_cols=5, fps=1):
        ...
        self.last_action=''
        self._init_pygame()
        self.package_picked = False  # Track package status
    ...
    def _init_pygame(self):
        ...
        img = pygame.image.load(file_name)
        self.package_img = pygame.transform.scale(img, self.cell_size)
    ...
    def reset(self, seed=None):
        ...
        while True:
            ...
            # Ensure it doesn't overlap with the robot
            if self.target_pos != self.robot_pos:
                break

        # Initialize package position, it shouldn't overlap with robot's or target's position
        while True:
            self.package_pos = [
                random.randint(0, self.grid_rows - 1),
                random.randint(0, self.grid_cols - 1)
            ]
            if self.package_pos != self.robot_pos and self.package_pos != self.target_pos:
                break
        
        self.package_picked = False  # Reset package status
    ...
    def render(self):
        ...
        for r in range(self.grid_rows):
            for c in range(self.grid_cols):
                ...
                elif [r, c] == self.package_pos and not self.package_picked:
                    print(GridTile.PACKAGE, end=' ')
                else:
        ...
        for r in range(self.grid_rows):
            for c in range(self.grid_cols):
                ...
                if [r, c] == self.package_pos and not self.package_picked:
                    # Draw package if not picked
